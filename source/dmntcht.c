...
Result dmntchtGetCheatProcessEvent(Event *event) {
  Handle evt_handle;
  Result rc
      = serviceDispatch(&g_dmntchtSrv, 65001, .out_handle_attrs = { SfOutHandleAttr_HipcCopy }, .out_handles = &evt_handle, );
...
Result dmntchtReadCheatProcessMemory(u64 address, void *buffer, size_t size) {
  ...
  return serviceDispatchIn(&g_dmntchtSrv, 65102, in, .buffer_attrs = { SfBufferAttr_Out | SfBufferAttr_HipcMapAlias },
                           .buffers = { { buffer, size } }, );
}
...
Result dmntchtWriteCheatProcessMemory(u64 address, const void *buffer, size_t size) {
  ...
  return serviceDispatchIn(&g_dmntchtSrv, 65103, in, .buffer_attrs = { SfBufferAttr_In | SfBufferAttr_HipcMapAlias },
                           .buffers = { { buffer, size } }, );
}
